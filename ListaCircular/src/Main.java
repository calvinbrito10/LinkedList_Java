...
public class Main {
	...
	private static final String[] pessoas = { "Lucas Kauer", "Castiçal", "Cruela", "Cartenha", "Juliãna", "Monekids",
			"Guilherme Autista", "Tiago Autista", "Tiagão", "Agnaldo", "Rodrigo Remor", "Monica (sem) Py", "Cantu",
			"Alexandra", "Filipe", "Mariãna", "Jesus", "Gabriel Mostarda", "André", "Marcelino Pão e Vinho", "Pedro",
	...
	private static Lista<String> josephus(int N, int M, int comeco) {
		...
		for (int i = 0; i < N; i++) {
			...
			while (!podeInserir) {
				...
				if (podeInserir)
					pessoinhas.inserirNoFim(pessoa);
				else
					pessoa = pessoas[aleatorio.nextInt(pessoas.length)];
			}
		}
